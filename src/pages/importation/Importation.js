...
export default function Importation() {
    ...
    axios
      .post(
        `${process.env.REACT_APP_BASE_URL}/historic2/`,
        {
          ...
          tvadgrkc: subTotal2,
          total: totalT,
          qteauthentificationconnaissement: authKnoledgeQuantity,
          qteconventionnel50tconsignation: firstConventionQuantity,
          qteconventionnelmilieuconsignation: secondConventionQuantity,
          qteconventionnel500tconsignation: thirdConventionQuantity,
          qtevehiculeconsignation: firstVehiculeQuantity,
          qteconteneur20consignation: firstContainer20Quantity,
          qteconteneur40consignation: firstContainer40Quantity,
          qteconteneur20tracking: secondContainer20Quantity,
          qteconteneur40tracking: secondContainer40Quantity,
          qteconteneur20equipement: thirdContainer20Quantity,
          qteconteneur40equipement: thirdContainer40Quantity,
          qtefraiscorrectionmanifesteequipement: manifestCorrectionQuantity,
          qtefraislettregarantiesimpleequipement: letterGuaranteeQuantity,
          qteconventionnelnavigation: redevanceConventionalQuantity,
          qtevehiculenavigation: redevanceVehiculeQuantity,
          qteconteneur20navigation: redevanceContainer20Quantity,
          qteconteneur40navigation: redevanceContainer40Quantity,
          user: user.name,
          vessel: vesselb,
          dates: date,
          date: new Date()
  ...
  // }, []);
  const [polTab, setPolTab] = useState([]);
  const [polo, setPol] = useState(null);
  // const [pod, setPod] = useState(null);
  ...
  const [vesselb, setVessel] = useState(null);
  ...
  useEffect(() => {
    setTotalUS(Math.round((totalT / dollar) * 10) / 10);
  }, [totalT, dollar]);
    ...
    <Page title="Importation | LMC App">
      <Container>
        ...
        <Card className="card-botton-2-wrapper">
          <div>
            <TextField label="Montant Total [Fc]" variant="outlined" value={totalT} />
          </div>
          <div>
            <TextField label="Montant Total [Us]" variant="outlined" value={totalUS} />
          </div>
          <div>
            <ReactToPrint
              trigger={() => (
